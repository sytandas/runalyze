...
with open(output_file, 'w', newline='') as csvfile:
    ...
    for filename in os.listdir(input_dir):
        ...
        # avg_hr = np.mean([h for h in heart_rate_vector if h is not None]) if heart_rate_vector else None
        # avg_cad = np.mean([c for c in cadence_vector if c is not None]) if cadence_vector else None
